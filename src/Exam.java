...
public class Exam {
  ...
  public static void showSelectedQuestions(Sual[] ALL) {
    int start = 1;
    int end = num;
    int count = 0;
    int xal = 0;
    ...
    System.out.println();
    System.out.println("-------------------------------");
    shuffled = randomiseArray(ALL, start - 1, end - 1, count);
    for (int i = 0; i < count; ++i) {
      ...
      if (cavab == s.correctAnswer) {
        ...
        System.out.println("XAL : " + xal);
        String correct = s.answers[s.correctAnswer - 1].getText();
  ...
  public static Sual[] randomiseArray(Sual[] arr, int start, int end, int count) {
    Random r = new Random();
    Sual[] newArray = new Sual[count];
    ...
    for (int i = 0; i < count; ++i) {
      ...
      Sual s = arr[randomHolder.get(i)];
      for (int j = 0; j < 5; j++) {
        ans[j] = new String(s.answers[j].getText());
      }
      newArray[i] = new Sual(s.question.getText(), ans);
  ...
  public static Sual[] prepareQuestionArray() throws Exception {
    ...
    for (int i = 0; i < num; ++i) {
      String head = new String(sc.nextLine().strip());
      String[] ans = new String[5];

      for (int j = 0; j < 5; ++j) {
        ans[j] = new String(sc.nextLine().strip());
      }
      allQuestions[i] = new Sual(head, ans);
